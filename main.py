...
def _build_first_review_prompt(repo_full, pr_number, pr_summary, sender, comment_body):
    ...
    return (
        ...
        f"**评审标准:**\n"
        f"- 用中文撰写评审\n"
        f"- 优先使用行级评论（post_line_review），将问题精确到代码行\n"
        f"- 行级评论按严重程度标注：🔴 严重 / 🟡 建议 / 🟢 良好实践\n"
        f"- 整体总结中给出概括性评价和统计\n"
        f"- 给出具体的修改建议和代码示例\n\n"
        f"**最终结论（重要！必须明确选择）:**\n"
        f"- 如果有 🔴 严重问题或必须修改的 🟡 建议 → 使用 `REQUEST_CHANGES`\n"
        f"- 如果代码质量良好，无严重问题 → 使用 `APPROVE`\n"
        f"- 如果只是建议性的改进意见 → 使用 `COMMENT`\n"
        f"- 不要总是用 COMMENT，根据实际情况选择 APPROVE 或 REQUEST_CHANGES"
    )
...
def _build_rereview_prompt(repo_full, pr_number, pr_summary, sender, comment_body):
    ...
    return (
        ...
        f"- 对新发现的问题按严重程度标注：🔴 严重 / 🟡 建议\n"
        f"- 整体总结中说明修复情况统计\n\n"
        f"**最终结论（重要！根据修复情况选择）:**\n"
        f"- 所有问题已修复，代码质量良好 → 使用 `APPROVE`\n"
        f"- 仍有未修复的严重问题 → 使用 `REQUEST_CHANGES`\n"
        f"- 大部分已修复，仅有小建议 → 使用 `COMMENT` 并 APPROVE"
    )
